...
SRC = os.path.join(HERE, "fb-harvest.json")
OUT = os.path.join(HERE, "fb")
...
def main():
    ...
    sheet.save(os.path.join(HERE, "fb-sheet.jpg"), quality=78)
    json.dump([os.path.basename(k[0]) for k in keep], open(os.path.join(HERE, "fb-keep.json"), "w"))
    print("sheet written")
